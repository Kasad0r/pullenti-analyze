...
public class ContentAnalyzeWhapper {
    ...
    private void _analizeContentWithContainers(FragToken root, java.util.ArrayList<InstrToken1> _lines, int topLevel, FragToken _topDoc) {
        ...
        for (int i = 0; i <= _lines.size(); i++) {
            ...
            if (li == null || (((n < nums.size()) && li == nums.get(n)))) {
                if (blk.size() > 0) {
                    if (fr == null) {
                        fr = FragToken._new1242(blk.get(0).getBeginToken(), blk.get(blk.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
                        if (blk.size() == 1) 
                            fr.itok = blk.get(0);
            ...
            if ((n < nums.size()) && li == nums.get(n)) {
                n++;
                fr = FragToken._new1243(li.getBeginToken(), li.getEndToken(), li, li.isExpired);
                root.children.add(fr);
                ...
                if (li.getBeginToken() != li.numBeginToken && li.numBeginToken != null) 
                    fr.children.add(FragToken._new1244(li.getBeginToken(), li.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true, li));
                ...
                if (li.numEndToken != li.getEndToken() && li.numEndToken != null) {
                    if (!li.allUpper && ((((li.hasVerb && names == 0 && li.getEndToken().isCharOf(".:"))) || li.getEndToken().isChar(':')))) 
                        fr.children.add(FragToken._new1242(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
                    else {
                        FragToken frName = FragToken._new1244(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                    ...
                    FragToken frName = FragToken._new1244(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                    fr.children.add(frName);
                    fr.name = FragToken.getRestoredNameMT(frName, false);
                    i = correctName(fr, frName, _lines, i);
                    names++;
            ...
            if (li.typ == InstrToken1.Types.EDITIONS && blk.size() == 0 && fr != null) 
                fr.children.add(FragToken._new1242(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.EDITIONS));
            else 
                blk.add(li);
        }
    }
        ...
        if (fr.name != null && (fr.name.length() < 100)) {
            for (; (i + 1) < _lines.size(); i++) {
                ...
                if (lii.getBeginToken().getWhitespacesBeforeCount() > 15) 
                    break;
                if (lii.getBeginToken().isValue("НЕТ", null) || lii.getBeginToken().isValue("ОТСУТСТВОВАТЬ", null)) 
                    break;
    ...
    private boolean _analizeChapterWithoutKeywords(FragToken root, java.util.ArrayList<InstrToken1> _lines, FragToken _topDoc) {
        ...
        for (int i = 0; i <= _lines.size(); i++) {
            ...
            if ((n < nums.size()) && li == nums.get(n)) {
                ...
                fr = FragToken._new1251(li.getBeginToken(), li.getEndToken(), li);
                childs.add(fr);
                fr.kind = com.pullenti.ner.instrument.InstrumentKind.CHAPTER;
                NumberingHelper.createNumber(fr, li);
                if (li.numEndToken != li.getEndToken() && li.numEndToken != null) {
                    if (li.hasManySpecChars) 
                        fr.children.add(FragToken._new1252(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
                    else {
                        FragToken frName = FragToken._new1244(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                        fr.children.add(frName);
                        fr.name = FragToken.getRestoredNameMT(frName, false);
    ...
    private void _addCommentOrEdition(FragToken fr, InstrToken1 li) {
        if (li.typ == InstrToken1.Types.COMMENT) 
            fr.children.add(FragToken._new1252(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.COMMENT, li));
        else if (li.typ == InstrToken1.Types.EDITIONS) {
            FragToken edt = FragToken._new1252(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.EDITIONS, li);
            fr.children.add(edt);
    ...
    private void _analizeContentWithoutContainers(FragToken root, java.util.ArrayList<InstrToken1> _lines, boolean isSubitem, boolean isPreamble, boolean isKodex) {
        ...
        if (root.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSE || ((root.kind == com.pullenti.ner.instrument.InstrumentKind.CHAPTER && docTyp == com.pullenti.ner.decree.DecreeKind.CONTRACT))) {
            ...
            if (_lines.get(0).numbers.size() == 0 && docTyp != com.pullenti.ner.decree.DecreeKind.CONTRACT) {
                ...
                for (int ii = 0; ii < _lines.size(); ii++) {
                    ...
                    if (part == null) {
                        part = FragToken._new1259(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CLAUSEPART, parts.size() + 1);
                        parts.add(part);
                    }
            ...
            if (_lines.get(ii).typ == InstrToken1.Types.NOTICE) {
                ...
                if (((li.numbers.size() == 1 && com.pullenti.unisharp.Utils.stringsEq(li.numbers.get(0), "1"))) || ((li.numbers.size() == 0 && ii == (_lines.size() - 1)))) {
                    for (int j = ii; j < _lines.size(); j++) {
                        li = _lines.get(j);
                        FragToken not = FragToken._new1252(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NOTICE, li);
                        notices.add(not);
                        if (li.numBeginToken != null && li.getBeginToken() != li.numBeginToken) 
                            not.children.add(FragToken._new1261(li.getBeginToken(), li.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true));
                        if (li.numbers.size() > 0) 
                            NumberingHelper.createNumber(not, li);
                        if (not.children.size() > 0) 
                            not.children.add(FragToken._new1252((li.numEndToken != null ? li.numEndToken : li.getBeginToken()), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
                    }
        ...
        if (nums == null) {
            ...
            for (InstrToken1 li : _lines) {
                ...
                if (li.typ == InstrToken1.Types.INDEX) {
                    FragToken ind = FragToken._new1252(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDEX, li);
                    ...
                    if (!li.indexNoKeyword) {
                        for (; tt != null && tt.endChar <= li.endChar; tt = tt.getNext()) {
                            if (tt.isNewlineAfter()) {
                                ind.children.add(FragToken._new1264(li.getBeginToken(), tt, com.pullenti.ner.instrument.InstrumentKind.NAME, true));
                                tt = tt.getNext();
                                break;
                            }
                        }
                    ...
                    boolean isTab = false;
                    for (; tt != null && tt.endChar <= li.endChar; tt = tt.getNext()) {
                        InstrToken1 it1 = InstrToken1.parse(tt, true, null, 0, null, false, 0, false);
                        if (it1 == null) 
                            break;
                        if ((!isTab && it1.endChar == li.endChar && tt.isTableControlChar()) && it1.getLengthChar() > 100) {
                            InstrToken1 it2 = InstrToken1.parse(tt.getNext(), true, null, 0, null, false, 0, false);
                            if (it2 == null) 
                                break;
                            it1 = it2;
                            tt = tt.getNext();
                        ...
                        if (tt.getReferent() instanceof com.pullenti.ner.decree.DecreePartReferent) {
                            ...
                            it1 = InstrToken1.parse(tt, true, null, 0, null, false, 0, false);
                        }
                        FragToken indItem = FragToken._new1242(tt, it1.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDEXITEM);
                        ind.children.add(indItem);
                        FragToken nam = null;
                        if (it1.numEndToken != null && it1.numEndToken != it1.getEndToken()) {
                            if (it1.getBeginToken() != it1.numBeginToken) 
                                indItem.children.add(FragToken._new1261(it1.getBeginToken(), it1.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true));
                            NumberingHelper.createNumber(indItem, it1);
                            indItem.children.add((nam = FragToken._new1261(it1.numEndToken.getNext(), it1.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true)));
                            InstrToken1 it2 = InstrToken1.parse(it1.getEndToken().getNext(), true, null, 0, null, false, 0, false);
                            if ((it2 != null && (it1.getEndToken().getNext() instanceof com.pullenti.ner.TextToken) && it2.numbers.size() == 0) && it2.titleTyp == InstrToken1.StdTitleType.UNDEFINED && !it1.getEndToken().getNext().isTableControlChar()) {
                                InstrToken1 it3 = InstrToken1.parse(it2.getEndToken().getNext(), true, null, 0, null, false, 0, false);
                            ...
                            indItem.children.add((nam = FragToken._new1261(it1.getBeginToken(), it1.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true)));
                ...
                if (last != null && last.kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) 
                    ...
                    root.children.add((last = FragToken._new1252(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li)));
            }
            if (!isPreamble) {
        ...
        for (; i < _lines.size(); i++) {
            ...
            if (li.numEndToken != li.getEndToken() && li.numEndToken != null) 
                ...
                fr.children.add(FragToken._new1264(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true));
            }
            if (blk.size() > 0) {
                fr.setEndToken(blk.get(blk.size() - 1).getEndToken());
            ...
            if (parts.get(0).name == null && parts.get(parts.size() - 1).name != null) {
                if (com.pullenti.unisharp.Utils.stringsEq(parts.get(1).name, "МОТИВИРОВОЧНАЯ") && !parts.get(0).isNewlineAfter()) {
                    ...
                    if (parts.get(0).children.size() == 0) 
                        parts.get(0).children.add(FragToken._new1252(parts.get(0).getBeginToken(), parts.get(0).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, parts.get(0).itok));
                }
            }
    ...
    private FragToken _createDirectivePart(java.util.ArrayList<InstrToken1> _lines) {
        FragToken res = FragToken._new1242(_lines.get(0).getBeginToken(), _lines.get(_lines.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.DOCPART);
        ...
        if (head.size() > 0) {
            FragToken frHead = FragToken._new1242(head.get(0).getBeginToken(), head.get(head.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
        ...
        res.children.add(FragToken._new1276(_lines.get(i).getBeginToken(), _lines.get(i).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.DIRECTIVE, _lines.get(i).value, _lines.get(i)));
            ...
            for (FragToken ch : root.children) {
                if (ch.kind == com.pullenti.ner.instrument.InstrumentKind.CHAPTER || ch.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSE) {
                    ...
                    if (ch.children.get(i).kind != com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
                        ...
                        sect = FragToken._new1261(ch.children.get(i).getBeginToken(), ch.children.get(i).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.SECTION, true);
                        sect.name = (String)com.pullenti.unisharp.Utils.cast(sect.value, String.class);
                        sect.value = null;
                        sect.children.add(FragToken._new1242(sect.getBeginToken(), sect.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME));
                        newChilds.add(sect);
                        ...
                        for (com.pullenti.ner.Token tt = la.getEndToken(); tt != null && tt.beginChar > la.beginChar; tt = tt.getPrevious()) {
                            if (tt.isNewlineBefore()) {
                                ...
                                if ((tt.getPrevious().endChar - la.beginChar) < 20) 
                                    break;
                                nextSect = FragToken._new1261(tt, la.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.SECTION, true);
                                nextSect.name = (String)com.pullenti.unisharp.Utils.cast(nextSect.value, String.class);
                                nextSect.value = null;
                                nextSect.children.add(FragToken._new1242(tt, la.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME));
                                break;
                            }
                        }
                        if (nextSect == null) 
    ...
    private void _correctNames(FragToken root, FragToken parent) {
        ...
        if (tt != null) {
            if (parent != null && parent.isExpired) {
                ...
                if (tt.isValue("УТРАТИТЬ", "ВТРАТИТИ") && tt.getNext() != null && tt.getNext().isValue("СИЛА", "ЧИННІСТЬ")) 
                    root.isExpired = true;
            }
        }
        for (FragToken ch : root.children) {
    ...
    private static java.util.ArrayList<FragToken> _splitContentByIndents(FragToken fr, int num) {
        if (fr.kind != com.pullenti.ner.instrument.InstrumentKind.CONTENT && fr.kind != com.pullenti.ner.instrument.InstrumentKind.LISTITEM && fr.kind != com.pullenti.ner.instrument.InstrumentKind.PREAMBLE) {
            ...
            if (fr.getBeginToken().isValue("АБЗАЦ", null)) {
                ...
                for (int i = num; i <= next; i++) {
                    res0.add(FragToken._new1281(fr.getBeginToken(), fr.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, i, true, fr.referents));
                }
                return res0;
            }
        ...
        for (com.pullenti.ner.Token tt = t0; tt != null && tt.endChar <= fr.endChar; tt = tt.getNext()) {
            if (tt.endChar == fr.endChar) {
                ...
                continue;
            FragToken re = FragToken._new1259(t0, tt, com.pullenti.ner.instrument.InstrumentKind.INDENTION, num);
            num++;
    ...
    private void _analizePreamble(FragToken root) {
                    ...
                    if (t.isNewlineBefore()) {
                        if (t.getPrevious().isCharOf(".:;") && t.getPrevious().getPrevious() != null && ((t.getPrevious().getPrevious().isValue("НИЖЕСЛЕДУЮЩИЙ", null) || t.getPrevious().getPrevious().isValue("ДОГОВОР", null)))) {
                            ...
                            if (itt1 != null && !itt1.hasVerb && (itt1.endChar < ch.endChar)) {
                                FragToken clau = FragToken._new1242(t, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CHAPTER);
                                ...
                                FragToken nam = FragToken._new1244(t, itt1.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, itt1);
                                clau.children.add(nam);
                                clau.name = FragToken.getRestoredNameMT(nam, false);
                                clau.children.add(FragToken._new1242(itt1.getEndToken().getNext(), ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
        ...
        if (cntCou == 1) {
            ...
            FragToken prm = FragToken._new1242(root.children.get(0).getBeginToken(), root.children.get(i - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.PREAMBLE);
            for (int j = 0; j < i; j++) {
