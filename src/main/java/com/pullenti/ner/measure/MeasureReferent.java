    ...
    public java.util.ArrayList<Double> getDoubleValues() {
        ...
        for (com.pullenti.ner.Slot s : getSlots()) {
            if (com.pullenti.unisharp.Utils.stringsEq(s.getTypeName(), ATTR_VALUE) && (s.getValue() instanceof String)) {
                double d;
                com.pullenti.unisharp.Outargwrapper<Double> wrapd1624 = new com.pullenti.unisharp.Outargwrapper<Double>();
                boolean inoutres1625 = com.pullenti.ner.measure.internal.MeasureHelper.tryParseDouble((String)com.pullenti.unisharp.Utils.cast(s.getValue(), String.class), wrapd1624);
                d = (wrapd1624.value != null ? wrapd1624.value : 0);
                if (inoutres1625) 
                    res.add(d);
            }
        }
        ...
        for (com.pullenti.ner.Slot s : getSlots()) {
            if (com.pullenti.unisharp.Utils.stringsEq(s.getTypeName(), ATTR_VALUE)) {
                if (s.getValue() instanceof String) 
                    vals.add((String)com.pullenti.unisharp.Utils.cast(s.getValue(), String.class));
