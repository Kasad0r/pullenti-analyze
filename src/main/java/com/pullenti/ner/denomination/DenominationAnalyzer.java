            ...
            for (com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
                ...
                if (ot != null && (ot.get(0).item.referent instanceof DenominationReferent)) {
                    if (this.checkAttach(ot.get(0).getBeginToken(), ot.get(0).getEndToken())) {
                        com.pullenti.ner.ReferentToken rt = new com.pullenti.ner.ReferentToken((DenominationReferent)com.pullenti.unisharp.Utils.cast(ot.get(0).item.referent, DenominationReferent.class), ot.get(0).getBeginToken(), ot.get(0).getEndToken(), null);
                        kit.embedToken(rt);
                        t = rt;
                        continue;
                    }
