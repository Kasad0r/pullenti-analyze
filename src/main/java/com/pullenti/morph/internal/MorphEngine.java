...
public class MorphEngine {
            ...
            for (String n : names) {
                if (com.pullenti.unisharp.Utils.endsWithString(n, rsname, true)) {
                    ...
                    try (com.pullenti.unisharp.Stream stream = com.pullenti.morph.internal.properties.Resources.getStream(n)) {
                        ...
                        m_LazyBuf = MorphSerializeHelper.deserializeAll(stream, this, false, true);
                    }
                    return true;
                }
            }
    ...
    private void _loadTreeNode(MorphTreeNode tn) {
        synchronized (m_Lock) {
            m_LazyBuf.seek(tn.lazyPos);
            MorphSerializeHelper.deserializeMorphTreeNodeLazy(m_LazyBuf, tn, this);
            tn.lazyPos = 0;
        }
    }
        ...
        for (i = 0; i <= word.length(); i++) {
            ...
            if (tn.rules != null) {
                ...
                for (MorphRule r : tn.rules) {
                    com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<MorphRuleVariant>> wrapmvs14 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<MorphRuleVariant>>();
                    boolean inoutres15 = com.pullenti.unisharp.Utils.tryGetValue(r.variants, wordEnd, wrapmvs14);
                    mvs = wrapmvs14.value;
                    if (inoutres15) {
                        if (wordBegin == null) {
                            if (i == word.length()) 
            ...
            com.pullenti.unisharp.Outargwrapper<MorphTreeNode> wraptn16 = new com.pullenti.unisharp.Outargwrapper<MorphTreeNode>();
            boolean inoutres17 = com.pullenti.unisharp.Utils.tryGetValue(tn.nodes, ch, wraptn16);
            tn = wraptn16.value;
            if (!inoutres17) 
                break;
        }
        ...
        if (needTestUnknownVars) {
            ...
            for (i = word.length() - 1; i >= 0; i--) {
                ...
                if (tn.nodes == null) 
                    break;
                MorphTreeNode next;
                com.pullenti.unisharp.Outargwrapper<MorphTreeNode> wrapnext18 = new com.pullenti.unisharp.Outargwrapper<MorphTreeNode>();
                boolean inoutres19 = com.pullenti.unisharp.Utils.tryGetValue(tn.nodes, ch, wrapnext18);
                next = wrapnext18.value;
                if (!inoutres19) 
                    break;
                tn = next;
                if (tn.lazyPos > 0) 
    ...
    public java.util.ArrayList<com.pullenti.morph.MorphWordForm> getAllWordforms(String word) {
        ...
        for (i = 0; i <= word.length(); i++) {
            ...
            if (tn.nodes == null || i >= word.length()) 
                break;
            short ch = (short)word.charAt(i);
            com.pullenti.unisharp.Outargwrapper<MorphTreeNode> wraptn20 = new com.pullenti.unisharp.Outargwrapper<MorphTreeNode>();
            boolean inoutres21 = com.pullenti.unisharp.Utils.tryGetValue(tn.nodes, ch, wraptn20);
            tn = wraptn20.value;
            if (!inoutres21) 
                break;
        }
    ...
    private void processProperVariants(String word, java.util.ArrayList<com.pullenti.morph.MorphWordForm> res, boolean geo) {
        ...
        for (i = word.length() - 1; i >= 0; i--) {
            ...
            if (tn.nodes == null) 
                break;
            MorphTreeNode next;
            com.pullenti.unisharp.Outargwrapper<MorphTreeNode> wrapnext28 = new com.pullenti.unisharp.Outargwrapper<MorphTreeNode>();
            boolean inoutres29 = com.pullenti.unisharp.Utils.tryGetValue(tn.nodes, ch, wrapnext28);
            next = wrapnext28.value;
            if (!inoutres29) 
                break;
            tn = next;
            if (tn.lazyPos > 0) 
    ...
    public com.pullenti.morph.MorphMiscInfo registerMorphInfo(com.pullenti.morph.MorphMiscInfo var) {
        ...
        com.pullenti.unisharp.Outargwrapper<com.pullenti.morph.MorphMiscInfo> wrapv30 = new com.pullenti.unisharp.Outargwrapper<com.pullenti.morph.MorphMiscInfo>();
        boolean inoutres31 = com.pullenti.unisharp.Utils.tryGetValue(m_VarsHash, key, wrapv30);
        v = wrapv30.value;
        if (inoutres31) 
            return v;
